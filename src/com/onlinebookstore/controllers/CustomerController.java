...
public class CustomerController {
  ...
  private static void viewCartAndCheckout(CustomArrayList<Book> shoppingCart, CustomArrayList<Integer> quantities) {
    ...
    while (true) {
        ...
        switch (cartOption) {
            case 1 -> {
                removeBookFromCart(shoppingCart, quantities);
                viewCartAndCheckout(shoppingCart, quantities); // Recalculate total price after removing a book
                return; // Exit the method after updating the cart
            }
            case 2 -> {
